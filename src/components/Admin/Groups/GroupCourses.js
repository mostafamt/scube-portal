...
export default function GroupCourses(props) {
  ...
  const courseColumns = [
    { field: "col1", headerName: "Course Title", width: 300 },
    { field: "col2", headerName: "Description", width: 350 },
    { field: "col3", headerName: "version", width: 100 },
    { field: "col4", headerName: "Language", width: 100 },
    { field: "col5", headerName: "Creation Date", width: 200 },
  ];
  ...
  for (let i = 0; i < courses.length; i++) {
    courseRows.push({
      id: courses[i]._id,
      col1: courses[i].title,
      col2: courses[i].description,
      col3: courses[i].version,
      col4: courses[i].language,
      col5: courses[i].createdAt,
    });
  }
  ...
  useEffect(() => {
    axios
      .get("https://apis.eduedges.com/api/lom/courses", { headers })
      .then((res) => {
        setCourses(res.data);
      })
      .catch((err) => {
      ...
      if (
        ...
        axios
          .post(
            `${
              config.apiURL
            }/cognito/${config.UserPool.getUserPoolId()}/groups/${group}/users`,
            data,
            { headers }
          )
          .then((res) => {
      ...
      if (groupName === "") {
        ...
        axios
          .post(
            `${
              config.apiURL
            }/cognito/${config.UserPool.getUserPoolId()}/groups`,
  ...
  const handleDeleteUsers = async (names) => {
    if (names.length === 0) {
      ...
      axios
        .delete(
          `${config.apiURL}/cognito/${config.UserPool.getUserPoolId()}/users`,
          { data: { names } },
          { headers }
        )
        .then(() => {
  ...
  const handleDelete = async (username) => {
    axios
      .delete(
        `${
          config.apiURL
        }/cognito/${config.UserPool.getUserPoolId()}/users/${username}`,
        { headers }
      )
      .then((res) => {
        removeUser(username);
            ...
            >
              ...
              >
                <Grid container spacing={3}>
                  {/* <Grid item xs={12}>
                    <Typography sx={{ fontSize: 20 }}>Group Courses</Typography>
                    <Divider /> 
                  </Grid> */}
                  <Grid
                    item
